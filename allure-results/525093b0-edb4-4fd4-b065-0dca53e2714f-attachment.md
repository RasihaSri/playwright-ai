...
- Name: 04_Session_Storage\Task_Invalid_Login_Free_Trial.spec.ts >> Verify invalid login for free trial page
- Location: tests\04_Session_Storage\Task_Invalid_Login_Free_Trial.spec.ts:3:5
...
```ts
  ...
  2  | 
  3  | test ('Verify invalid login for free trial page', async ({ page}) => {
  4  | 
  ...
  7  | 
  8  |     await page.getByRole ("textbox", { name: "Business Email "}, { exact: true }).fill("dsdad@gmail.com");
> 9  |     await page.getByRole ("checkbox", { name: "gdpr_consent_checkbox" }).check();
     |                                                                          ^ Error: locator.check: Test timeout of 30000ms exceeded.
  10 |     await page.getByRole ("button", { name: "Create a Free Trial Account" }).click();
  11 | 
  12 |     await expect(page.getByText("gmail.com doesn't look like a business domain. Please use your business email.")).toBeVisible();
  13 |     console.log("Error message is visible, invalid login is verified");
  14 | })
```