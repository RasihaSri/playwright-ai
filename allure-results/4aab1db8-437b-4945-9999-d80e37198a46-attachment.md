...
Call log:
  - waiting for locator('xpath=(//*[text()="Uttar Pradesh"])[2]')
    - locator resolved to <tspan dy="5.599995048522942">Uttar Pradesh</tspan>
...
```ts
  ...
  15 |         const key = "Uttar Pradesh";
  16 |         // Zoom only the SVG map area
  17 |         await page.evaluate(() => {
  18 |             const el = document.getElementById('admin1_map_inner');
  19 |             if (el) {
  20 |                 el.style.transform = 'scale(1.5)';
  21 |                 el.style.transformOrigin = 'top left';
  22 |             }
  23 |         });
  24 | 
  25 |         const allStates = page.locator('//div[@id="admin1_map_inner"]//*[name()="svg"]//*[name()="text"]');
  26 |         console.log("All States: " + await allStates.allTextContents());
  27 | 
  28 |         const count = await allStates.count();
  29 |         for (let i = 0; i < count; i++) {
  30 |             const state = allStates.nth(i);
  31 |             const text = await state.textContent();
  32 |             if (text?.includes(key)) {
  33 |                 // await state.scrollIntoViewIfNeeded();
> 34 |                 await page.locator(`(//*[text()="${key}"])[2]`).click();
     |                                                                 ^ Error: locator.click: Test timeout of 30000ms exceeded.
  35 |                 break;
  36 |             }
  37 |         }
  38 | 
  39 |     //      const states = await page
  40 |     //   .locator(
  41 |     //     `//div[@id='admin1_map_inner']//*[name()='svg']//*[name()='text' and contains(@class,'sm_label')]`,
  42 |     //   )
  43 |     //   .allTextContents();
  44 | 
  45 |     // // Printing all 36 states and clicking on UP
  46 |     // for (const state of states) {
  47 |     //   if (state.trim() === "Uttar Pradesh") {
  48 |     //     await page
  49 |     //       .locator(`//*[name()='path' and contains(@class,'INUP')]`)
  50 |     //       .click();
  51 |     //   }
  52 |     // }
  53 | 
