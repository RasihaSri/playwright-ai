...
Call log:
  - waiting for getByText('⌘V', { exact: true }).first()
    - locator resolved to <span class="ctx-shortcut">⌘V</span>
    ...
    - main [ref=e231]:
      ...
      - generic [ref=e241]:
        - article [ref=e243]:
          ...
          - generic [ref=e263]: "{ \"action\": \"copy\", \"label\": \"Copy\", \"target\": \"ctx-target\", \"at\": \"2:33:58 AM\" }"
...
```ts
  ...
  9  | 
  10 |     for (const option of allOptions) {
  11 |         await page.locator ('span.context-menu-one').first().click({ button: 'right' });
> 12 |         await page.getByText(option, { exact: true}).first().click();
     |                                                              ^ Error: locator.click: Test timeout of 30000ms exceeded.
  13 |         expect (page.locator("#output")).toHaveText(option);
  14 |     }
  15 | 
  16 | 
