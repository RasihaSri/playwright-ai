...
```
Error: locator.fill: Error: strict mode violation: getByPlaceholder('enter name') resolved to 2 elements:
    1) <input value="" disabled name="test" class="nameFld" placeholder="First Enter name"/> aka getByRole('textbox', { name: 'First Enter name' })
    2) <input value="" id="kils" type="text" autocorrect="off" autocomplete="off" spellcheck="false" autocapitalize="off" title="user name field" placeholder="enter name"/> aka getByRole('textbox', { name: 'user name field' })

Call log:
  - waiting for getByPlaceholder('enter name')
                ...
                - generic [ref=f2e8]:
                  ...
                  - generic [ref=f2e16]:
                    - link "Sign in to Google" [ref=f2e17] [cursor=pointer]:
                      - /url: https://accounts.google.com/Login?continue=https%3A%2F%2Fdocs.google.com%2Fforms%2Fd%2Fe%2F1FAIpQLSeguyGBfogdUO6IRfxJapNxTJQTaAEWYfcd5984L0T_q-0Syg%2Fviewform%3Ffbzx%3D2395683813632762236
...
```ts
  ...
  12 | 
> 13 |         await page.getByPlaceholder("enter name").fill('Rasiha');
     |                                                   ^ Error: locator.fill: Error: strict mode violation: getByPlaceholder('enter name') resolved to 2 elements:
  14 |         await page.getByPlaceholder("Enter pizza name").fill('Dominos');
  15 | 
  16 |         await page.locator("#training").fill('Playwright');
  17 |         await page.getByTitle("user password field").fill('password123');
  18 | 
