...
```ts
  ...
  15 |         const allStates: Locator = page.locator('//div[@id="admin1_map_inner"]//*[name()="svg"]//*[name()="text"]');
  16 |         allStates.scrollIntoViewIfNeeded();
  17 |         console.log("All States: " + await allStates.allTextContents());
  18 | 
  19 |         for (const state of await allStates.all()) {
  20 |             if ((await state.textContent())?.includes("Uttar Pradesh")) {
> 21 |                 await state.click();
     |                             ^ Error: locator.click: Test ended.
  22 |                 break;
  23 |             }
  24 |         }
