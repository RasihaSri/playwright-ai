...
Call log:
  - waiting for locator('//div[@id="admin1_map_inner"]//*[name()="svg"]//*[name()="text"]').nth(33)
    - locator resolved to <text x="307.536" y="251.808" opacity="1" stroke="none" fill="#ffffff" stroke-width="0" font-size="16px" font-weight="bold" font="10px "Arial"" font-family="Arial" text-anchor="middle" class="sm_label sm_label_INUP" transform="matrix(0.688,0,0,0.688,95.9512,78.5641)">…</text>
...
```ts
  ...
  17 |         const allStates = page.locator('//div[@id="admin1_map_inner"]//*[name()="svg"]//*[name()="text"]');
  18 |         console.log("All States: " + await allStates.allTextContents());
  19 | 
  20 |         const count = await allStates.count();
  21 |         for (let i = 0; i < count; i++) {
  22 |             const state = allStates.nth(i);
  23 |             const text = await state.textContent();
  24 |             if (text?.includes("Uttar Pradesh")) {
  25 |                 // await state.scrollIntoViewIfNeeded();
> 26 |                 await state.click();
     |                             ^ Error: locator.click: Test timeout of 30000ms exceeded.
  27 |                 break;
  28 |             }
  29 |         }
