...
```ts
  ...
  6  | 
  7  |     // Fill QA profile (example selectors, adjust as needed)
> 8  |     await page.getByLabel('Full Name').fill('Rasiha Sri');
     |                                        ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  9  |     await page.getByLabel('Email').fill('rasiha@thetestingacademy.com');
  10 |     await page.getByLabel('Role').selectOption('QA');
  11 |     await page.getByLabel('Experience').fill('5');
  12 |     await page.getByLabel('Location').fill('Remote');
  13 | 
  14 |     // Add Git Hub profile (example, adjust selectors as needed)
  15 |     await page.getByRole('button', { name: /Add Profile/i }).click();
  16 |     await page.getByLabel('Profile Name').fill('Git Hub');
  17 |     await page.getByLabel('Profile Type').selectOption('Git');
  18 |     await page.getByLabel('Hub URL').fill('http://localhost:4444/wd/hub');
  19 |     await page.getByRole('button', { name: /Save Profile/i }).click();
  20 | 
  21 |     // Take a screenshot after adding profiles
  22 |     await page.screenshot({ path: 'tests/07_Web_Tables/qa_gridhub_profiles.png', fullPage: true });
  23 | });
  24 | 
  25 | 
```